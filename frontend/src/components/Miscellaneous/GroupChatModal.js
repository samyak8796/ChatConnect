...
const GroupChatModal = ({children}) => {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const { data } = await axios.post(`/api/chat/group`,{ name: groupChatName,
            ...
            <ModalContent>
                ...
                <ModalBody display="flex" flexDir="column" alignItems="center">
                    ...
                    {loading ? ( <div>Loading...</div> ) : (
                        searchResult ?.slice(0, 4).map((user) => (<UserListItem key={user._id} 
                            user={user} handleFunction={() => handleGroup(user)}/>))
                    )}
                </ModalBody>
                <ModalFooter>
